 ...
 * @returns {Promise<{recId:string, codigo:string, via:'principal'|'asignado'|'interconsulta'|'demo', escritura:boolean}>}
...
async function autorizarPaciente(codigoMedico, pacienteCode, { requiereEscritura = false } = {}) {
  ...
  if (f[CAMPO_ES_DEMO] === true) {
    ...
    return { recId: registro.id, codigo: pacienteCode, via: 'demo', escritura: false };
  }
  ...
  if (medico && linkPrincipal.includes(medico.id)) {
    return { recId: registro.id, codigo: pacienteCode, via: 'principal', escritura: true };
  }
