...
(function () {
  ...
  function startObserver() {
    ...
    observer.observe(document.body, { 
      childList: true, 
      subtree: true,
      characterData: true  // Detecta cambios en textContent/nodeValue
    });
    ...
    }, 250); // Cada 250ms para atrapar animaciones y contenido dinámico muy rápidamente

    // Observar específicamente elementos que cambian por animaciones/JS (como .decode)
    if (window.MutationObserver) {
      var textChangeObs = new MutationObserver(function (mutations) {
        if (current === 'es') return;
        for (var i = 0; i < mutations.length; i++) {
          if (mutations[i].type === 'characterData') {
            translateTextNodes(mutations[i].target.parentNode || mutations[i].target, current);
          }
        }
      });
      // Observar todo el documento para cambios en nodos de texto
      textChangeObs.observe(document.body, {
        characterData: true,
        subtree: true,
        childList: false
      });
