...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const fields = {
      ...
      'Estado': 'Pendiente',
      'Notas': codigoCortesia ? `Código de cortesía asignado: ${codigoCortesia}` : '',
    };
    ...
    const solicitudId = recordId || (createData.records && createData.records[0] && createData.records[0].id);

    // Alertar a los médicos maestros por Telegram (si ya vincularon su cuenta)
    // — con botones reales de un solo toque, no texto libre a interpretar.
    ...
    const mensaje =
      ...
      (codigoCortesia ? `\n🎓 Ya tiene un código de cortesía asignado (${codigoCortesia}) — esto es solo informativo, la decisión de afiliarlo sigue siendo tuya.\n` : '') +
      `\n¿Autorizas su afiliación? Al autorizar, NOVA genera su código CCMED- automáticamente.`;

    const botones = {
      inline_keyboard: [[
        { text: '✅ Autorizar', callback_data: `autorizar_solicitud:${solicitudId}` },
        { text: '❌ Rechazar', callback_data: `rechazar_solicitud:${solicitudId}` },
      ]],
    };
    ...
    await Promise.all(
      CODIGOS_MAESTROS.map(async (codigo) => {
        try {
          ...
          if (chatId) await sendTelegramMessage(chatId, mensaje, botones);
        } catch (err) {
