...
module.exports = async function handler(req, res) {
  ...
  if (action === 'kiosco_crear_paciente') {
    try {
      const { staffCodigo, nombreCompleto, edad, sexo, telefono } = req.body;
      ...
      // Médico que abrió la sesión (ya verificado en el paso 1 del kiosco)
      ...
      const medicoRecordId = medData.records?.[0]?.id || null;

      // Siguiente código CC-PAC- disponible
      const listRes = await fetch(`https://api.airtable.com/v0/${BASE_ID}/${TBL_PAC}?fields[]=C%C3%B3digo%20de%20paciente`, { headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` } });
      const listData = await listRes.json();
      const numeros = (listData.records || [])
        .map(r => parseInt((r.fields['Código de paciente'] || '').replace('CC-PAC-', ''), 10))
        .filter(n => !isNaN(n));
      const siguienteNum = (numeros.length ? Math.max(...numeros) : 0) + 1;
      const nuevoCodigo = `CC-PAC-${String(siguienteNum).padStart(6, '0')}`;
    ...
    if (herramientaMedico) {
      ...
      if (toolAlta && typeof toolAlta.input?.nombre_completo === 'string') {
        try {
          ...
          // Siguiente código CC-PAC- disponible (se listan los existentes y se
          // calcula el máximo + 1 — la tabla es chica, esto es barato).
          const listRes = await fetch(`https://api.airtable.com/v0/${BASE_ID}/${TBL_PAC}?fields[]=C%C3%B3digo%20de%20paciente`, { headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` } });
          const listData = await listRes.json();
          const numeros = (listData.records || [])
            .map(r => parseInt((r.fields['Código de paciente'] || '').replace('CC-PAC-', ''), 10))
