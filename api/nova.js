...
function buildHerramientaFichaConsulta() {
  return {
    ...
    description: 'Úsala SOLO cuando el médico dicte o describa datos clínicos de un paciente que está atendiendo ahora, con intención de llenar la ficha de consulta (edad, diagnóstico, signos vitales, motivo, plan). No la uses para preguntas generales, de protocolos o consultas que no describen a un paciente concreto. IMPORTANTE: si en el mismo mensaje el médico también pide contenido largo (ej. un plan nutricional completo con menús), NO lo generes aquí — llena la ficha normalmente y en "mensaje" dile que use la pestaña Nutrición del expediente para eso (función en desarrollo). Mantén "mensaje" y todos los campos breves y concretos; nunca redactes documentos largos dentro de esta herramienta.',
    input_schema: {
      type: 'object',
      properties: {
        mensaje: {
