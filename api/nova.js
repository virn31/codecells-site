...
module.exports = async function handler(req, res) {
  ...
  if (action === 'generar_plan_semanal') {
    try {
      const { pacienteRecordId, medicoCode, peso: pesoReq, talla: tallaReq, edad: edadReq, sexo: sexoReq, factorActividad, objetivo, preferencias, dias, objetivosSemana, tipoDieta, planNutricional } = req.body;
        ...
        for (const [horario, proporcion] of Object.entries(DISTRIBUCION)) {
          ...
          const conScore = candidatos.map(p => {
            ...
            if (kcalBase > 0) {
              ...
              score = diffCalorico + penalizacion + bonoObjetivosSemana(p);
            }
            return { p, score };
          });
      ...
      return res.status(200).json({
        ...
        tipo_dieta: tipoDietaEfectivo || 'estandar',
